...
class Dynamics():
    ...
    def univariate_plot(self,figsize = (20,20)):
        ...
        ax.plot([i for i in np.arange(start,end+.1, .1)], [self.functions[0](i) for i in np.arange(start,end+.1, .1)], color = 'b');
        ax.plot([i for i in np.arange(start,end+.1, .1)], [i for i in np.arange(start,end+.1, .1)], color = 'black');
        ax.vlines(x = dynamic_path[0], ymin = start, ymax = self.functions[0](dynamic_path[0]), color = 'orange');
        for i, init in enumerate(dynamic_path):
            ax.vlines(x = init, ymin = init, ymax = self.functions[0](init), color = 'orange')
            try:
                ax.hlines(y = self.functions[0](init), xmin = dynamic_path[i], xmax = dynamic_path[i+1], color = 'orange');
            except:
                pass
        ax = axes[1]
    ...
    def bivariate_plot(self,figsize = (20,20)):
        x_path = self.X[0]
        y_path = self.X[1]
        fig, axes = plt.subplots(2,1, figsize = figsize)
        ax = axes[0]
    ...
    def trivariate_plot(self, figsize = (20,20)):
        x_path = self.X[0]
        y_path = self.X[1]
        z_path = self.X[2]
